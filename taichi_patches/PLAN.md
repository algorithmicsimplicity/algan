...
| 44 | Path-traced emission slot the NEE table can sample; a BSDF-closure contract for custom scatter under the path tracer (Q5) | n/a | **scratch (Algan), later**: the emission slot is small and safe after row 21 (`path_tracer.py:246-256` reads built-in slots only); the BSDF closure (eval/sample/pdf with MIS) is a renderer design, not plumbing. Custom scatter itself now *runs* under the path tracer as a delta lobe (`DESIGN_path_tracer_roadmap.md` §9, `test_custom_scatter_renders_as_a_delta_continuation`); what row 44 would add is the sampling density that would let NEE cover it | — |
