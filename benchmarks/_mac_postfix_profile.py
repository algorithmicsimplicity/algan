...
def arguments():
    ...
    parser.add_argument(
        "--coarse",
        action="store_true",
        help="Time function scopes in warm runs without ATen dispatch or cProfile",
    )
...
def child(args):
    ...
    assert can_use_bloom_taichi(torch.device(args.child)), (
        "Production bloom gate must engage"
    )
    timer = Timers()
    ...
    def wavefront(*a, **kw):
        ...
        if native is not None:
            native.algan_graph_phase(
                (2 * chunk - 1 if chunk <= 2 else 5)
                if timer.enabled and state.get("native_active")
                else -1
            )
    ...
    for run in range(1, args.runs + 1):
        state.update(
            ...
            batches=0,
            arenas=[],
            native_active=run == args.profile_run,
        )
        timer.rows.clear()
        timer.slow.clear()
        ...
        if cpu_profile is not None:
            ...
            with (output / "host_profile.txt").open("w") as stream:
                pstats.Stats(cpu_profile, stream=stream).sort_stats(
                    "tottime"
                ).print_stats(70)
                pstats.Stats(cpu_profile, stream=stream).sort_stats(
                    "cumulative"
        ...
        if detailed:
            if native is not None:
                ...
                ti.sync()
                (output / "metal_buffers.json").write_text(
                    native.algan_metal_stats().decode()
                )
            (output / "timings.json").write_text(json.dumps(timer.data(), indent=2))
            if native is not None:
                stats = json.loads(native.algan_graph_stats())
