...
def child():
    ...
    def checked_select(
        ...
        *,
        ...
        candidates = (
            torch.arange(self.capacity, dtype=torch.int32) if scan_pool else source_cpu
        )
        ...
        print(
            ...
            good,
            "status",
            torch.unique(state_cpu[:, 2], return_counts=True),
            "hits",
            torch.unique(state_cpu[:, 3], return_counts=True),
            flush=True,
        )
        if not good:
            torch.save(
