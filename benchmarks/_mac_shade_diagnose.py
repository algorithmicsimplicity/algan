...
def child():
    ...
    if limit:
        ...
        kernel = kernel.replace(
            ...
            "ashp: ti.types.ndarray(), active_begin: int):",
        )
        kernel = kernel.replace(
            "for i in range(num_active):", "for i in range(active_begin, num_active):"
        )
        path.write_text(source[:begin] + kernel + source[end:])

    (OUT / "shaders").mkdir(parents=True, exist_ok=True)
    os.environ["QD_SHADER_DUMP_DIR"] = str(OUT / "shaders")
    faulthandler.enable()
    ...
    def traverse(*args):
        ...
        print(
            "DIAG_TRAVERSE_END", traversal["calls"], json.dumps(traversal), flush=True
        )
    ...
    def captured_shade(*args):
        nonlocal shade_calls
        params = wf._WAVEFRONT_SHADE_PARAMS
        ...
        before = named["rs_int"].cpu()
        real_shade(*args)
        after = named["rs_int"].cpu()
        shade_calls += 1
        print(
            "DIAG_SHADE_STATE",
            shade_calls,
            ...
            "before_status",
            torch.unique(before[:, 2], return_counts=True),
            "after_status",
            torch.unique(after[:, 2], return_counts=True),
            "hits",
            torch.unique(before[:, 3], return_counts=True),
            "before_sample",
            before[:8].tolist(),
            "after_sample",
            after[:8].tolist(),
        ...
        if capture is not None:
            ...
            torch.save(
                {"inputs": capture, "outputs": outputs}, OUT / "shade-capture.pt"
            )
        if shade_calls >= 2:
            print("DIAG_SHADE_CAPTURED", flush=True)
            os._exit(0)

    tracer.wavefront_shade = captured_shade
...
def snapshot(proc, arm, number):
    ...
    try:
        ...
        return
    command(
        ["/usr/bin/sample", str(proc.pid), "3", "-file", str(tag) + "-sample.txt"],
        Path(str(tag) + "-sample-command.txt"),
        timeout=15,
    )
    command(["/usr/bin/vm_stat"], Path(str(tag) + "-vm-stat.txt"))
    command(["/usr/sbin/sysctl", "vm.swapusage"], Path(str(tag) + "-swap.txt"))
    command(
        ["/bin/ps", "-axo", "pid,ppid,state,%cpu,rss,vsz,comm"],
...
def supervise(arm, cap):
            ...
            while proc.poll() is None:
                for key, _ in sel.select(timeout=1):
                    ...
                    while b"\n" in pending:
                        ...
                        if (
                            "DIAG_CHUNK " in text
                            or "DIAG_TRAVERSE_END" in text
                            or "chunk " in text
                            and "begins at" in text
                        ):
                ...
                if now - last_progress > 240 or now - started > 1200:
                    reason = (
                        "no_progress" if now - last_progress > 240 else "time_budget"
                    )
    ...
    command(
        [
            ...
            "compact",
            "--predicate",
            'eventMessage CONTAINS[c] "GPU" OR eventMessage CONTAINS[c] "AGX" OR eventMessage CONTAINS[c] "Paravirtual"',
        ],
        OUT / f"{arm}-system-log.txt",
        timeout=30,
    )
...
def main():
    ...
    command(
        [
            ...
            "swift",
            "-e",
            'import Metal; for d in MTLCopyAllDevices() { print("METAL_DEVICE", d.name, "unified", d.hasUnifiedMemory, "maxBufferLength", d.maxBufferLength, "recommended", d.recommendedMaxWorkingSetSize, "allocated", d.currentAllocatedSize) }',
        ],
        OUT / "metal-devices.txt",
        timeout=60,
    )
