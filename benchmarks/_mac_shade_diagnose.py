...
def child():
    ...
    def captured_shade(*args):
        ...
        capture = {name: value.cpu() if isinstance(value, torch.Tensor) else value
                   for name, value in named.items()} if shade_calls == 0 else None
        before = named["rs_int"].cpu()
        real_shade(*args)
        after = named["rs_int"].cpu()
        shade_calls += 1
        print("DIAG_SHADE_STATE", shade_calls, "n", args[1],
              "changed", int((before != after).sum()),
              "before_status", torch.unique(before[:, 2], return_counts=True),
              "after_status", torch.unique(after[:, 2], return_counts=True),
              "hits", torch.unique(before[:, 3], return_counts=True),
              "before_sample", before[:8].tolist(), "after_sample", after[:8].tolist(),
              flush=True)
        if capture is not None:
            outputs = {name: named[name].cpu() for name in
                       ("rs_ro", "rs_rd", "rs_acc", "rs_sca", "rs_int", "pix_accum", "rs_alloc")}
            torch.save(dict(inputs=capture, outputs=outputs), OUT / "shade-capture.pt")
        if shade_calls >= 2:
