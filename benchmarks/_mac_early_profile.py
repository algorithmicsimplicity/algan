...
def child(args):
    ...
    os.environ.update(
        ALGAN_RENDER_DEVICE=args.child,
    ...
    if args.bloom_ab:
        ...
        from algan.rendering.post_processing import (
            bloom_kernels_taichi as bloom_kernels,
        )
    ...
    def arena_init(self, portion, device=None, managed=True, *, num_bytes=None):
        ...
        with timer.span(
            "memory.arena_allocation" if managed else "memory.unmanaged_init"
        ):
            original_arena(
    ...
    def get_tensor(self, *a, **kw):
        ...
        with timer.span(
            "memory.arena_view" if self.managed else "memory.pool_allocation"
        ):
            return original_get(self, *a, **kw)
    ...
    for name in (
        ...
        "_select_largest_fitting_fetched_prefix",
        "_prepared_batch_fits_render_arena",
        "_get_batch_of_primitives",
        "_prewarm_render_batch",
        "_prepare_batch_on_worker",
        "_materialize_render_state",
    ):
    ...
    class OperatorTimes(TorchDispatchMode):
        def __torch_dispatch__(self, func, types, args=(), kwargs=None):
            ...
            def metadata(value):
                ...
                if isinstance(
                    value,
                    (str, int, float, bool, type(None), torch.dtype, torch.device),
                ):
                    return str(value)
                return type(value).__name__

            signature = (
                ";".join(metadata(x) for x in args[:3])
                + ":"
    ...
    def wavefront(*a, **kw):
        ...
        if chunk == 3 and mode is not None:
            ...
            emit(
                "profile_checkpoint",
                early_window=elapsed,
                write_seconds=time.perf_counter() - checkpoint,
            )
        if native is not None:
            native.algan_graph_phase(
                2 * chunk - 1 if timer.enabled and chunk <= 2 else -1
            )
        ...
        try:
            ...
            if native is not None:
                native.algan_graph_phase(
                    2 * chunk if timer.enabled and chunk <= 2 else -1
                )
    ...
    emit(
        ...
        taichi=str(getattr(ti, "__version__", "unknown")),
        ...
        env={
            ...
            if k.startswith(("ALGAN_", "PYTORCH_MPS_"))
        },
    )
    for run in range(1, args.runs + 1):
        ...
        emit(
            ...
            arenas=state["arenas"],
            pool=pool(),
            bloom_candidate=candidate,
            bloom_times=dict(bloom_times),
        )
        if detailed:
            (output / "timings.json").write_text(json.dumps(timer.data(), indent=2))
            if native is not None:
    ...
    if args.bloom_ab:
        ...
        for channels in (4, 5):
            ...
            for use_candidate in (False, True):
                ...
                result = bloom_module.bloom_filter(
                    input_tensor, memory=memory, scale_factor=64
                ).cpu()
                ...
                emit(
                    ...
                    launches=launches,
                    staged=staged,
                    effective_resize_factor=3,
                )
                if candidate and args.child == "mps" and (launches != 2 or staged):
                    raise AssertionError(
                        "Parity fixture must launch both Metal resize kernels without staging"
                    )
            indices = [0, 1, 2, 4] if channels == 5 else [0, 1, 2]
            first, second = [x[..., indices] for x in answers]
            difference = (first - second).abs()
            quantized = [
                (x * 255).clamp(0, 255).to(torch.int16) for x in (first, second)
            ]
            byte_difference = (quantized[0] - quantized[1]).abs()
            emit(
                "bloom_parity",
                channels=channels,
                max_float=difference.max().item(),
                mean_float=difference.mean().item(),
                max_byte=byte_difference.max().item(),
                changed_bytes=(byte_difference != 0).sum().item(),
                values=byte_difference.numel(),
            )
