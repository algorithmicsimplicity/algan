...
* **The offline kernel cache is keyed on the compiled IR**, so an edit to
  `_material_reflectance` or `_scatter_impl` recompiles by itself; clear it with
  `clear_cached_kernels()` only when the A/B is measuring compile time.
