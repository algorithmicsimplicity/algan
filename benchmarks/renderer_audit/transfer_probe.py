...
CONDITIONS = {
    "unlit": ([], {"type": "basic"}),
    "ambient1": ([{"type": "ambient", "color": [1, 1, 1], "intensity": 1.0}],
                 {"type": "standard", "roughness": 1.0, "metalness": 0.0}),
    "directional1": ([{"type": "directional", "direction": [0, 0, -1],
    ...
    for tonemap in (True, False):
        for cond, (lights, mat) in CONDITIONS.items():
            ...
            for level in LEVELS:
                ...
                spec["name"] = name
                spec["lights"] = lights
                spec["objects"][0]["material"] = dict(
                    mat, color=[level, level, level])
                spec_path = OUT / f"{name}.json"
                spec_path.write_text(json.dumps(spec))
                cmd = [sys.executable, str(_HERE / "algan_render.py"),
                       str(spec_path), "--out", str(OUT), "--aa", "1"]
                if not tonemap:
