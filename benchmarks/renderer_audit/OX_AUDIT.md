...
> **Historical source audit, not a current defect list.** Color management,
> Fresnel/IOR handling and several material responses changed after the snapshot
> below. Current source status is summarized in
> [RENDERER_WORK_QUEUE.md](../../RENDERER_WORK_QUEUE.md); remaining priorities are
> in [TODO.md](../../TODO.md). The algebra and original verdicts are retained to
> explain those changes, not to claim the same defects remain.
