...
def child(arm):
    ...
    import runpy, random, numpy as np, torch
    ...
    if arm=="mps-sorted":
        from _mps_reorder_probe import install
        install()
    if arm=="mps-unsorted":
    ...
    result=ns["Scene"].save_frame(str(OUT/(arm+".png")),video_settings=ns["PRESET"],at=0.15,overwrite=True)
    print("PARITY_FRAME",arm,str(result.output_path),time.perf_counter()-start,flush=True)

def main():
    OUT.mkdir(parents=True,exist_ok=True)
    for arm in ("mps-sorted","mps-unsorted","cpu"):
        env=dict(os.environ,ALGAN_RENDER_DEVICE="cpu" if arm=="cpu" else "mps",
                 ALGAN_TORCH_COMPILE="0",ALGAN_MPS_HOST_SHARE="0",
                 ALGAN_USE_DAEMON="0",PYTHONUNBUFFERED="1")
        with (OUT/(arm+".txt")).open("w") as log:
            result=subprocess.run([sys.executable,__file__,"--child",arm],cwd=ROOT,env=env,
                                  stdout=log,stderr=subprocess.STDOUT,timeout=450)
        print((OUT/(arm+".txt")).read_text(),flush=True)
        if result.returncode: return result.returncode
    from PIL import Image
    import numpy as np
    images={arm:np.asarray(Image.open(OUT/(arm+".png"))).astype(np.int16)
            for arm in ("mps-sorted","mps-unsorted","cpu")}
    comparisons={}
    for arm in ("mps-unsorted","cpu"):
        d=np.abs(images["mps-sorted"]-images[arm])
        comparisons[arm]=dict(max=int(d.max()),mean=float(d.mean()),
                              p999=float(np.quantile(d,.999)),
