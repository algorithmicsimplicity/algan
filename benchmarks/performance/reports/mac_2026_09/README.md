...
| 5 | **Take the torch↔Taichi fences once per region, not once per launch** (landed, differently: see [SHARED_QUEUE.md](SHARED_QUEUE.md)) | `mps_zero_copy.install_zero_copy_launch` took `torch.mps.synchronize()` before and `ti.sync()` after **every** converted launch, which serialized the GPU completely. Rather than fencing per region, Quadrants now dispatches on torch's own Metal command queue, so Metal's in-order execution orders the two frameworks and the fences are one non-blocking torch commit per launch | measured on the runner: 9.5 s of a 59 s warm UHD render was inside the two fences |
