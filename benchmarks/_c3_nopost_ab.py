"""The C.3 A/B with post-processing disabled: is bloom the amplifier?"""
...
def main():
    ...
    render(False, "shapes_off_nopost.mp4")
    render(True, "shapes_on_nopost.mp4")

    import cv2
    import numpy as np

    a = cv2.VideoCapture(str(OUT / "shapes_off_nopost.mp4"))
    b = cv2.VideoCapture(str(OUT / "shapes_on_nopost.mp4"))
    i = 0
    nz = []
    while True:
        oa, fa = a.read()
    ...
    for f, m, c, c2 in nz:
        print(f"  frame {f:3d} max {m:2d} px>0 {c:5d} px>2 {c2}")
