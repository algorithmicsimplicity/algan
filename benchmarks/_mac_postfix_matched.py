...
def main():
    ...
    for tag, device, runs, timeout in blocks:
        ...
        print(
            "MATCHED_START " + json.dumps({"tag": tag, "command": command}), flush=True
        )
        started = time.perf_counter()
        # Keep the original child output even if it exceeds its time budget.
        with (output / (tag + ".log")).open("w") as stream:
            process = subprocess.Popen(
                command, stdout=stream, stderr=subprocess.STDOUT, env=dict(os.environ)
            )
            try:
