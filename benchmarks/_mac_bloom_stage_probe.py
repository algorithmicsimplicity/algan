...
def comparison(name, actual, reference):
    ...
    for index in where[:8].tolist():
        ix = tuple(index)
        examples.append(
            {"index": index, "actual": a[ix].item(), "reference": b[ix].item()}
        )
    row = {
        "name": name,
        "shape": list(a.shape),
...
for height, width, scale, channels in (
    (127, 193, 3, 3),
    (64, 80, 4, 4),
    (270, 480, 8, 3),
):
    ...
    for arm in ("fallback", "metal"):
        kernels.can_use_bloom_taichi = (
            lambda device, arm=arm: arm == "metal"
        ...
        print(
            "ENGAGEMENT",
            arm,
            zc.STATS["converted_launches"] - before["converted_launches"],
            flush=True,
        )
...
original_down, original_up, original_fft = (
    bloom._downsample_bloom,
    bloom._upsample_bloom,
    bloom.fft_conv1d,
)
captures = {}
arm = "cpu"
...
bloom._downsample_bloom, bloom._upsample_bloom, bloom.fft_conv1d = (
    down_capture,
    up_capture,
    fft_capture,
)
...
for arm in ("cpu", "fallback", "metal"):
    kernels.can_use_bloom_taichi = (
        lambda device, arm=arm: arm == "metal" and torch.device(device).type == "mps"
    )
    mem, tensor, prefix = allocation(
        "cpu" if arm == "cpu" else "mps", frames.shape, torch.uint8
    )
    tensor.copy_(frames)
    fft_index = 0
    captures[arm + "/final"] = (
        bloom.bloom_filter(tensor, memory=mem, scale_factor=64).cpu().clone()
    )
    assert (prefix.cpu() == 91).all()
for arm in ("fallback", "metal"):
