...
Renders one frame with
  ...
  * the same square with stroke_width 0 (must be unchanged by this work),
  * an unfilled circle and a Line (centred strokes, unchanged),
  * dense text (must not fuse as stroke_width rises).
...
def main():
    with Off():
        for i, bw in enumerate((0, 4, 12)):
            (
                Square(color=BLUE, stroke_color=RED, stroke_width=bw)
                .spawn()
                .scale(0.8)
                .move(LEFT * 5 + UP * 2.5 + RIGHT * 3.2 * i)
            )
        Circle(
            color=GREEN, stroke_color=YELLOW, stroke_width=8, filled=False
        ).spawn().scale(0.8).move(RIGHT * 5 + UP * 2.5)
        Line(LEFT * 2, RIGHT * 2, color=WHITE, stroke_width=8).spawn().move(
            LEFT * 3 + DOWN * 1
        )
        Text("minimum", stroke_color=RED, stroke_width=6).spawn().scale(2).move(
            DOWN * 2.5
        )
    Scene.wait()
    out = os.path.join(
