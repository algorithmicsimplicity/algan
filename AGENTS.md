...
- The **full-render baselines are per machine, not merely per device**: on a different CPU five of six scenes missed by 29–204 channel values, because `pn_criterion_kernel` runs under `fast_math` and borderline tessellation levels depend on the host CPU. `test_full_render_scene` therefore skips itself when `CI` is set (`ALGAN_RUN_FULL_RENDERS=1` overrides). `tests/fast` is portable and matched exactly on another machine — trust it.
