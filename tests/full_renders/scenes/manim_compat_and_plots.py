...
with Seq():
    ...
    with Sync(runtime=1.8):
        ...
        ApplyMatrix(grid, [[1.0, 0.6], [0.0, 1.0]], runtime=1.8)
        MoveAlongPath(traveller, path, runtime=1.8)
    ...
    with Sync(runtime=1.6):
        ApplyComplexFunction(grid, lambda z: z * (0.8 + 0.4j), runtime=1.6)
        MoveAlongPath(traveller, path, runtime=1.6)
        star.rotate(-50)
    Scene.wait(0.4)
