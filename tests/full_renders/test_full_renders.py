...
test_scenes_dir = os.path.join(cd, "scenes")
test_files = [[f] for f in os.listdir(test_scenes_dir) if f.endswith(".py")]
...
class TestOverseer(TestCase):
    def setUp(self):
        ...
        error_dir = os.path.join(cd, "output_errors")
    ...
    def test_algan_file(self, test_file):
        ...
        with Scene() as scene:
            importlib.import_module(f"scenes.{module_name}")
            scene.save_video(module_name)
