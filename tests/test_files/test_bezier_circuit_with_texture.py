...
p = torch.stack(
    (
        torch.stack(
            (UP, UP + RIGHT * 0.1, UP + RIGHT * 0.2, UP + RIGHT * -0.2 + DOWN * 0.5)
        ),
        torch.stack(
            (
                UP + RIGHT * -0.2 + DOWN * 0.5,
                UP + RIGHT * -0.3 + DOWN * 0.4,
                UP + RIGHT * -0.2 + DOWN * 0.1,
                UP,
            )
        ),
    )
...
def test_bezier_circuit_basic():
    ...
    with Sync(
        run_time=r
    ):  # TODO with run_time=8 is crash, with run_time=3 is visual glitch with bezier curve leaking outside it's boundary
        with Seq(run_time=r):
