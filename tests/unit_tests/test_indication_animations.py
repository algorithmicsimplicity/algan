...
@pytest.mark.parametrize(
    "animation",
    [
        lambda mob: Indicate(mob, runtime=0.75),
        lambda mob: Wiggle(mob, n_wiggles=2, runtime=0.75),
        lambda mob: Circumscribe(mob, runtime=0.75),
        lambda mob: Flash(mob, num_lines=4, runtime=0.75),
        lambda mob: FocusOn(mob, runtime=0.75),
        lambda mob: ShowPassingFlash(mob, runtime=0.75),
        lambda mob: ShowPassingFlashWithThinningStrokeWidth(
            mob, n_segments=3, runtime=0.75
        ),
        lambda mob: ApplyWave(mob, runtime=0.75),
    ],
    ids=[
        "Indicate",
        "Wiggle",
...
def test_indication_runtime_wins_over_the_enclosing_context(scene, animation):
    with Off():
        square = Square(stroke_width=4).spawn()
    start = _elapsed(scene)
    with Seq(runtime=None):
        animation(square)
    assert _elapsed(scene) - start == pytest.approx(0.75, abs=1e-6)


def test_blink_runtime_comes_from_its_on_and_off_times(scene):
    with Off():
        square = Square().spawn()
    start = _elapsed(scene)
...
def test_flash_registers_every_spoke_it_draws(scene):
    ...
    Flash(square, num_lines=7, runtime=0.5)
...
def test_focus_on_registers_its_spotlight_and_leaves_it_despawned(scene):
    with Off():
        square = Square().spawn()
    spotlight = FocusOn(square, runtime=0.5)
...
def test_circumscribe_registers_its_frame_for_every_fade_combination(scene, kwargs):
    ...
    Circumscribe(square, runtime=0.5, **kwargs)
...
def test_circumscribe_rejects_a_shape_it_cannot_build(scene):
    ...
    with pytest.raises(ValueError, match="Rectangle or Circle"):
        Circumscribe(square, shape=Group, runtime=0.5)
...
def test_flash_accepts_a_bare_point_as_well_as_a_mob(scene):
    with Off():
        Square().spawn()
    before = len(_actors_of_type(scene, Line))
    Flash(torch.tensor([0.5, 0.5, 0.0]), num_lines=5, runtime=0.5)
...
def test_show_passing_flash_uses_stroke_clones_and_restores_source(scene):
    ...
    ShowPassingFlash(circle, time_width=0.2, runtime=0.5)
    ShowPassingFlash(circle, time_width=0.2, runtime=0.5)
    end = _elapsed(scene)
...
@pytest.mark.parametrize(
    "animation",
    [
        lambda mob: Indicate(mob, runtime=0.5),
        lambda mob: Wiggle(mob, n_wiggles=2, runtime=0.5),
        lambda mob: ApplyWave(mob, runtime=0.5),
    ],
...
def test_indicate_flashes_the_colour_in_the_middle(scene):
    ...
    with Off():
        square = Square(color=BLUE).spawn()
    start = _elapsed(scene)
    Indicate(square, scale_factor=1.5, runtime=1.0)
...
def test_indicate_scales_a_composite_around_its_anchor(scene):
    ...
    start = _elapsed(scene)
    Indicate(group, scale_factor=1.5, runtime=1.0)
...
def test_indication_animations_accept_a_group(scene):
    ...
    start = _elapsed(scene)
    Indicate(group, runtime=0.5)
    ApplyWave(group, runtime=0.5)
