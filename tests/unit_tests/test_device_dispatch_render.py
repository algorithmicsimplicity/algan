        ...
        for enabled in (False, True, False):
            ...
            with Scene(video_settings=SMOKE_TEST) as scene:
                ...
                with Off():
                    Scene.clear_lights()
                    PointLight(
                        location=UP * 3 + RIGHT * 2, color=WHITE, intensity=1
                    ).spawn(animate=False)
                    material = (
                        MeshPhysicalMaterial(color=WHITE, roughness=0.12, ior=5)
                        if reflective
                        else MeshLambertMaterial(color=WHITE)
                    )
                ...
                result = scene.save_frame(
                    tmp_path / f"{reflective}_{len(frames)}.png",
                    video_settings=SMOKE_TEST,
                    overwrite=True,
                )
                assert (len(seen) > before) == enabled
                frames.append(
                    np.asarray(Image.open(result.output_path)).astype(np.int16)
                )
