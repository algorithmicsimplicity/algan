...
from algan.rendering.post_processing import bloom
...
@pytest.mark.parametrize(
    ("arch", "available", "installed", "expected"),
    [
...
def test_mps_bloom_requires_live_metal_imports(
    monkeypatch, arch, available, installed, expected
):
...
@pytest.mark.parametrize(
    ("batch", "height", "width", "scale", "channels"),
    [
        (1, 127, 193, 3, 3),
        (2, 64, 80, 4, 4),
        (1, 270, 480, 8, 3),
...
def test_full_bloom_matches_cpu_reference(monkeypatch, batch, channels):
    ...
    frames = torch.randint(
        1, 220, (batch, 127, 193, channels), dtype=torch.uint8, generator=generator
    )
    frames[..., 3] = 100
    ...
    def quantized(value):
        return value.mul(255).clamp(0, 255).to(torch.uint8).to(torch.int16)
