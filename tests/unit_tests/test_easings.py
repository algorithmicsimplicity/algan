...
``easings`` is a leaf module every animation goes through, and a rate
...
from algan import DEFAULT_EASING, Off, Scene, Square, Sync, easings
...
CURVES = sorted(
    name
    for name, value in vars(easings).items()
    if not name.startswith("_")
    and callable(value)
    and name not in {"inversed"}
    and getattr(value, "__module__", None) == easings.__name__
)
...
def test_easing_starts_at_zero_and_finishes_at_one(name):
...
def test_easing_is_finite_and_stays_inside_the_unit_interval(name):
    values = _evaluate(getattr(easings, name), SAMPLES)
...
def test_easing_never_runs_an_animation_backwards(name):
    values = _evaluate(getattr(easings, name), SAMPLES)
    assert (values.diff() >= -1e-5).all(), f"{name} is not monotone increasing"


@pytest.mark.parametrize("name", CURVES)
def test_easing_accepts_a_scalar_progress_tensor(name):
    # Contexts evaluate the curve on whatever shape the timeline produced,
    # including a single frame time.
    value = float(
        torch.as_tensor(getattr(easings, name)(torch.tensor(0.5))).reshape(-1)[0]
    )
...
def test_linear_is_the_identity_curve_not_a_copy_of_it():
    assert easings.linear is easings.identity
    assert _evaluate(easings.identity, SAMPLES) == pytest.approx(
        SAMPLES.double(), abs=1e-6
    )


def test_inversed_mirrors_a_curve_through_the_diagonal():
    mirrored = easings.inversed(easings.ease_out_expo)
    forward = _evaluate(easings.ease_out_expo, SAMPLES)
...
def test_smooth_takes_its_inflection_as_a_keyword_default():
    ...
    signature = inspect.signature(easings.smooth)
    assert signature.parameters["inflection"].default == 10.0
    sharp = _evaluate(easings.smooth, SAMPLES)
    gentle = torch.as_tensor(easings.smooth(SAMPLES, 2.0)).reshape(-1).double()
...
def test_a_context_easing_reshapes_the_path_but_not_its_endpoints(name):
