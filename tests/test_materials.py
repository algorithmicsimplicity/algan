...
Run directly:  python tests/test_materials.py
Or via pytest: python -m pytest tests/test_materials.py
"""
...
# Animation/geometry happen on CPU; keep these tests off the GPU and fast.
...
def test_base_defaults():
    m = Material()
    assert torch.as_tensor(m.color).reshape(-1)[:3].tolist() == [1.0, 1.0, 1.0]
...
def test_set_material_wires_shader_and_attrs():
    ...
    # Standard applies the (default white) material colour to the mob.
    assert _v(s.color)[:3] == [1.0, 1.0, 1.0]
...
def test_physical_mode_routes_surface_params():
    ...
    try:
        enable_ray_tracing(physical_lighting=True)
        s = Sphere().set_material(MeshStandardMaterial(metalness=1.0, roughness=0.3))
        assert abs(_v(s.reflectivity)[0] - 1.0) < 1e-6
        assert abs(_v(s.roughness)[0] - 0.3) < 1e-6
        assert "reflectivity" in s.shader_specific_param_names
    finally:
        set_physical_lighting(False)
        disable_ray_tracing()
    print("ok: physical mode routes metalness/roughness to RT surface params")


def test_no_raytracer_routing_in_default_mode():
    from algan import Sphere

    s = Sphere().set_material(MeshStandardMaterial(metalness=1.0, roughness=0.3))
    # Deterministic/rasterized default: must NOT turn the metal into a mirror.
    assert not hasattr(s, "reflectivity"), "metalness leaked to RT mirror param"
    print("ok: no RT surface-param routing without physical lighting")
