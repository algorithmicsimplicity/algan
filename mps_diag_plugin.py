...
def _rss_mb():
    try:
        raw = subprocess.check_output(["ps", "-o", "rss=", "-p", str(os.getpid())], text=True).strip()
        return int(raw) / 1024
    except Exception:
        return -1.0
...
def pytest_runtest_teardown(item, nextitem):
    ...
    if torch.backends.mps.is_available():
        ...
        cur = drv = -1.0
    print("FILEMEM", str(item.path), "rss_mb", round(_rss_mb(), 1), "mps_cur_mb", round(cur, 1), "mps_drv_mb", round(drv, 1), "zero_copy", _zero_copy(), flush=True)
