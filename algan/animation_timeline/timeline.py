...
class AttributeTimeline:
    ...
    def rematerialize_state_at_times(self, times, active_mob_ids=None, extra_rows=None):
        ...
        # ``pointer`` is the number of rows in use, and it is also the exclusive
        # bound on every row id in circulation: ``add`` hands out
        # ``arange(pointer, new_pointer)`` and every edit, range and endpoint
        # layout is built from those. Materializing ``pointer + 1`` rows -- as
        # this did -- appended a row nothing can address, which for a
        # colour-texture timeline is a whole spare image per frame.
