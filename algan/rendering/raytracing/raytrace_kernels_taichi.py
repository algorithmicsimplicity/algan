...
megakernel used when ``samples_per_pixel > 1``: ``path_trace_scene_stbvh``,
launching one thread per (frame, pixel, sample) path and accumulating
atomically into a float buffer that ``finalize_samples`` averages. The deterministic
