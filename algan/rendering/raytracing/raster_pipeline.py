...
def precompute_triangle_projection(
    ...
    valid = ((n2 > 1e-30) & (big_d.abs() > 1e-20))[:, None]
    valid = valid & (denom * sign[:, None, None] > 1e-9).all(-1)
    ...
    packed = torch.cat((sx, sy, inv_d, valid.to(sx.dtype).unsqueeze(-1)), -1)
...
def _screen_bbox(px, py, front, width, row_lo, row_hi):
    ...
    Camera-plane straddlers still fall back to the full current row band.  A
    future implementation should reject all-behind bounds and clip straddling
    triangles/polygons against the camera or near plane before projection.
    """
