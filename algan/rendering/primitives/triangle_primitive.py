...
def interpolate_triangle_corners(self, interpolation_coord, _property):
...
class TrianglePrimitive(RenderPrimitive):
    ...
    def interpolate_property(self, interpolation_coord, _property, repeats_inds):
        return interpolate_triangle_corners(
            self,
            interpolation_coord,
            self.expand_verts_to_frags(_property, repeats_inds.unsqueeze(-1), -3),
        )
