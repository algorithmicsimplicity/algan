...
class RenderPrimitive:
    ...
    def render_window(self, primitives, scene, window, save_image, time_start, time_end, object_start,
        ...
        try:
            out = self.get_tensor(
                (((window[2] - window[0]) * (window[3] - window[1])), 4 if not transparent_output else 5), torch.uint8)
    ...
    def render_(self, time_start, time_end, object_start, object_end, ray_origin, screen_point, screen_basis,
        ...
        all_ws = self.get_interpolation_coordinates(corners_locs, fragment_x, fragment_y, None)
