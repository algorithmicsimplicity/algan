...
from algan.geometry.geometry import project_point_onto_line_segment
from algan.logging.logger import LoggerManager
from algan.rendering.primitives.primitive import RenderPrimitive2D
...
class BezierCircuitPrimitive(RenderPrimitive2D):
    def __init__(
        ...
        if triangle_collection is not None:
            ...
            self.num_segments_per_circuit = torch.cat(
                [_.num_segments_per_circuit for _ in triangle_collection]
            ).to(device)
            self.num_segments_per_object = torch.stack(
                [_.num_segments_per_circuit.sum() for _ in triangle_collection]
            ).to(device)
            logger.log_message(
                f"np_o: {self.num_segments_per_object}, np_c: {self.num_segments_per_circuit}"
            )
            ...
            (
                ...
                (torch.stack(list(_), 1)).to(device)
                for _ in zip(
                    *(
                        broadcast_all(
                            (
    ...
    def get_windowed_bounding_boxes(
        ...
        def get_bounding_box_fragment_coords(x):
            ...
            LoggerManager.instance().set_class("rendering").log_message("bg3")
            ...
            LoggerManager.instance().set_class("rendering").log_message("done")
    ...
    def render_(
        ...
        select_time(self.corners_int)
        select_time(self.projected_distances)
        if corners.numel() == 0:
            return None
        ...
        select_time(self.portion_of_curve_drawn)
        ...
        LoggerManager.instance().set_class("rendering").log_message("got local windows")
        ...
        LoggerManager.instance().set_class("rendering").log_message("got local_to_bbox")
        ...
        LoggerManager.instance().set_class("rendering").log_message("got invalid mask")
        ...
        mob_center_for_frags = self.expand_verts_to_frags(
            squish(mob_center, 0, 1).squeeze(-2), object_to_fragment_gather_inds
        )
