...
class BezierCircuitPrimitive(RenderPrimitive2D):
    ...
    def get_windowed_bounding_boxes(self, bounding_corners, screen_width, screen_height, window_coords=None):
        ...
        def get_bounding_box_fragment_coords(x):
            ...
            arange_num_segments_per_oject = self.get_tensor([len(self.num_segments_per_object)], dtype=torch.long)
            torch.arange(len(self.num_segments_per_object), device=arange_num_segments_per_oject.device, out=arange_num_segments_per_oject)
            ...
            object_bounding_box_dimensions = self.get_tensor(object_bounding_corners_top_right.shape, dtype=object_bounding_corners_top_right.dtype)
            torch.subtract(object_bounding_corners_top_right, object_bounding_corners_bottom_left, out=object_bounding_box_dimensions)
            # object_bounding_box_num_pixels = object_bounding_box_dimensions.prod(-1, keepdim=True)
            object_bounding_box_num_pixels = self.get_tensor([*object_bounding_box_dimensions.shape[:-1], 1], dtype=torch.long)
            torch.prod(object_bounding_box_dimensions, -1, keepdim=True, out=object_bounding_box_num_pixels)
            ...
            arange_numel = self.get_tensor([object_bounding_box_num_pixels.numel()], dtype=torch.long)
            torch.arange(object_bounding_box_num_pixels.numel(), device=arange_numel.device, out=arange_numel)
            object_to_fragment_gather_inds = torch.repeat_interleave(
                arange_numel, object_bounding_box_num_pixels.view(-1), -1,
                output_size=num_fragments).unsqueeze(-1)
            ...
            torch.arange(num_fragments, device=object_fragment_inds.device, out=object_fragment_inds)
            ...
            object_offsets_flat = self.get_tensor(flattened_pixels.shape, dtype=flattened_pixels.dtype)
            cumsum_pixels = self.get_tensor(flattened_pixels.shape, dtype=flattened_pixels.dtype)
            ...
            temp_gathered = broadcast_gather(object_offsets, -2, object_to_fragment_gather_inds, keepdim=True, out=self.memory)
            torch.subtract(object_fragment_inds, temp_gathered, out=object_fragment_inds)
            ...
            object_bounding_box_dimensions_for_frags = broadcast_gather(squished_dims, -2, object_to_fragment_gather_inds, keepdim=True, out=self.memory)
            ...
            object_bounding_corners_bottom_left_for_frags = broadcast_gather(squished_corners, -2, object_to_fragment_gather_inds, keepdim=True, out=self.memory)
            ...
            object_fragment_x = self.get_tensor(object_fragment_inds.shape, dtype=object_fragment_inds.dtype)
            temp_remainder = self.get_tensor(object_fragment_inds.shape, dtype=object_fragment_inds.dtype)
            torch.remainder(object_fragment_inds, object_bounding_box_dimensions_for_frags[..., :1], out=temp_remainder)
            torch.add(temp_remainder, object_bounding_corners_bottom_left_for_frags[..., :1], out=object_fragment_x)
            ...
            object_fragment_y_bbox = self.get_tensor(object_fragment_inds.shape, dtype=object_fragment_inds.dtype)
            torch.floor_divide(object_fragment_inds, object_bounding_box_dimensions_for_frags[..., :1], out=object_fragment_y_bbox)
            # object_fragment_y = object_fragment_y_bbox + object_bounding_corners_bottom_left_for_frags[..., 1:]
            object_fragment_y = self.get_tensor(object_fragment_y_bbox.shape, dtype=object_fragment_y_bbox.dtype)
            torch.add(object_fragment_y_bbox, object_bounding_corners_bottom_left_for_frags[..., 1:], out=object_fragment_y)
    ...
    def project_to_screen(self, camera, light_sources):
        ...
        control_point_diffs = self.get_tensor(control_points[..., 1:, :].shape, dtype=control_points.dtype)
        torch.subtract(control_points[..., 1:, :], control_points[..., :-1, :], out=control_point_diffs)
        control_point_norms = self.get_tensor([*control_point_diffs.shape[:-1]], dtype=control_points.dtype)
        torch.norm(control_point_diffs, p=2, dim=-1, out=control_point_norms)
        control_net_lengths = self.get_tensor([*control_point_norms.shape[:-1]], dtype=control_points.dtype)
    ...
    def render_(self, time_start, time_end, object_start, object_end, ray_origin, screen_point, screen_basis,
        ...
        bbox_num_pixels = self.get_tensor([*object_bounding_box_dimensions_for_segments.shape[:-2], 1, object_bounding_box_dimensions_for_segments.shape[-1]], dtype=object_bounding_box_dimensions_for_segments.dtype)
        torch.prod(object_bounding_box_dimensions_for_segments, -2, keepdim=True, out=bbox_num_pixels)
        ...
        offsets = self.get_tensor([*object_bounding_box_dimensions.shape[:-1], 1], dtype=torch.long)
        ...
        broadcast_gather(left_intersection_counts, -1, row_start_ind, keepdim=True, out=row_start_counts)
        ...
        screen_basis_norm = self.get_tensor([*screen_basis.shape[:-1], 1], dtype=screen_basis.dtype)
        torch.norm(screen_basis, p=2, dim=-1, keepdim=True, out=screen_basis_norm)
        screen_basis_norm_sq = self.get_tensor(screen_basis_norm.shape, dtype=screen_basis.dtype)
        ...
        if self.num_texture_points > 0:
            ...
            grid_height = self.get_tensor(expo(self.grid_height).shape, dtype=torch.long)
            grid_width_float = self.get_tensor(expo(self.grid_width).shape, dtype=torch.float)
            grid_height_float = self.get_tensor(expo(self.grid_height).shape, dtype=torch.float)
            ...
            disps_from_mobs = self.get_tensor(proj_onto_mobs.shape, dtype=proj_onto_mobs.dtype)
            ...
            for w, x, y in [(w1, x_floor, y_floor), (w2, x_ciel, y_floor), (w3, x_floor, y_ciel), (w4, x_ciel, y_ciel)]:
                ...
                interpolated_colors = torch.addcmul(interpolated_colors, c, w, out=interpolated_colors)
