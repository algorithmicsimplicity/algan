...
from ..mobject.text.numbers import DecimalNumber, Integer
from ..mobject.text.tex_mobject import MathTex
from ..mobject.text.text_mobject import Paragraph
...
class Table(VGroup):
    ...
    def set_column_colors(self, *colors: Iterable[ParsableManimColor]) -> Self:
        ...
        for color, column in _zip(colors, columns, strict=False):
    ...
    def set_row_colors(self, *colors: Iterable[ParsableManimColor]) -> Self:
        ...
        """
        rows = self.get_rows()
        for color, row in _zip(colors, rows, strict=False):
            row.set_color(color)
        return self
