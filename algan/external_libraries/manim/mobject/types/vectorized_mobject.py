...
class VMobject(Mobject):
    ...
    def generate_rgbas_array(
        ...
        rgbas: FloatRGBA_Array = np.array(
            [
                ...
                for c, o in _zip(*make_even(colors, opacities), strict=True)
            ],
        )
    ...
    def match_style(self, vmobject: VMobject, family: bool = True) -> Self:
        ...
        if family:
            ...
            if len(submobs1) == 0:
                ...
                submobs2 = [vmobject]
            for sm1, sm2 in _zip(*make_even(submobs1, submobs2), strict=True):
                sm1.match_style(sm2)
        return self
    ...
    def _gen_subpaths_from_points(
        ...
        return (
            points[i1:i2]
            for i1, i2 in _zip(split_indices[:-1], split_indices[1:], strict=True)
            if (i2 - i1) >= nppcc
        )
    ...
    def get_anchors(self) -> list[Point3D]:
        ...
        return list(it.chain.from_iterable(_zip(s, e, strict=True)))
...
class VDict(VMobject, metaclass=ConvertToOpenGL):
    ...
    def add_key_value_pair(self, key: Hashable, value: VMobject) -> Self:
        ...
        if self.show_keys:
            ...
            from ...mobject.text.tex_mobject import Tex
