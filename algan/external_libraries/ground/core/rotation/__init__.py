...
PointRotatorAroundOrigin = Callable[[Point, None, None, Type[Point]], Point]
PointTranslatingRotator = Callable[[Point, None, None, None, None, Type[Point]], Point]
...
class Context:
    ...
    def rotate_segment_around_origin(
        ...
        return segment_cls(
            self.rotate_point_around_origin(segment.start, cosine, sine, point_cls),
            self.rotate_point_around_origin(segment.end, cosine, sine, point_cls),
        )
        ...
        return multisegment_cls(
            [
                self.rotate_translate_segment(
                    segment, cosine, sine, step_x, step_y, point_cls, segment_cls
                )
                for segment in multisegment.segments
            ]
        )
    ...
    def rotate_translate_polygon(
        ...
        return polygon_cls(
            self.rotate_translate_contour(
                polygon.border, cosine, sine, step_x, step_y, contour_cls, point_cls
            ),
            [
                self.rotate_translate_contour(
    ...
    def __init__(
        ...
        rotate_translate_point: PointTranslatingRotator,
    ) -> None:
