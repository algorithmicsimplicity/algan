...
def centroid(multipolygon: Multipolygon, point_cls: Type[Point]) -> Point:
    x_numerator, y_numerator, double_area = centroid_components(multipolygon.polygons)
...
def centroid_components(polygons: Sequence[Polygon]):
    ...
    for polygon in iterator:
        (polygon_x_numerator, polygon_y_numerator, polygon_double_area) = (
            polygon_centroid_components(polygon.border, polygon.holes)
        )
