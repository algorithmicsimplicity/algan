...
from algan.external_libraries.ground.core.primitive import rationalize, to_sign


def test(point: Point, first: Point, second: Point, third: Point):
    point_x, point_y = rationalize(point.x), rationalize(point.y)
    first_dx, first_dy = (
        rationalize(first.x) - point_x,
        rationalize(first.y) - point_y,
    )
    second_dx, second_dy = (
        rationalize(second.x) - point_x,
        rationalize(second.y) - point_y,
    )
    third_dx, third_dy = (
        rationalize(third.x) - point_x,
