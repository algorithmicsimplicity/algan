...
from .hints import Point, TernaryPointFunction


def to_convex_hull(
    points: Sequence[Point], orienteer: TernaryPointFunction[Orientation]
) -> List[Point]:
...
def _to_sub_hull(
    points: Iterable[Point], orienteer: TernaryPointFunction[Orientation]
) -> List[Point]:
    result = []
    for point in points:
        while len(result) >= 2:
