...
def point_squared_distance(box: Box, point: Point):
    return (
        _linear_interval_distance(
            rationalize(box.min_x), rationalize(box.max_x), rationalize(point.x)
        )
        ** 2
        + _linear_interval_distance(
            rationalize(box.min_y), rationalize(box.max_y), rationalize(point.y)
        )
        ** 2
    )
...
def segment_squared_distance(
    ...
    return (
        0
        if (
            (min_x <= segment_start.x <= max_x and min_y <= segment_start.y <= max_y)
            or (min_x <= segment_end.x <= max_x and min_y <= segment_end.y <= max_y)
        )
        else (
            (
                segment_point_squared_distance(
                    segment_start, segment_end, point_cls(min_x, min_y), dot_producer
                )
                if min_y == max_y
...
def _linear_interval_distance(min_coordinate, max_coordinate, coordinate):
    return (
        min_coordinate - coordinate
        if coordinate < min_coordinate
        else (coordinate - max_coordinate if coordinate > max_coordinate else 0)
    )
...
def _non_degenerate_segment_squared_distance(
    ...
    bottom_left, bottom_right = (point_cls(min_x, min_y), point_cls(max_x, min_y))
    ...
    return left_side_distance and min(
        bottom_side_distance, right_side_distance, top_side_distance, left_side_distance
    )
