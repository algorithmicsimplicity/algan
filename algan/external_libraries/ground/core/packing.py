...
def pack_mix(
    ...
    return (
        ...
        if (
            ((discrete is not empty) + (linear is not empty) + (shaped is not empty))
            >= 2
        )
        else (
            discrete
...
def pack_points(
    points: Sequence[Point], empty: Empty, multipoint_cls: Type[Multipoint]
) -> Maybe[Multipoint]:
    return multipoint_cls(points) if points else empty


def pack_segments(
    segments: Sequence[Segment], empty: Empty, multisegment_cls: Type[Multisegment]
