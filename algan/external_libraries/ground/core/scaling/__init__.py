...
from algan.external_libraries.ground.core.packing import (
    pack_mix,
    pack_points,
    pack_segments,
)
...
class Context:
    ...
    def scale_contour(
        ...
        return (
            ...
            if factor_x and factor_y
                ...
                contour, factor_x, factor_y, multipoint_cls, point_cls, segment_cls
            )
        )
    ...
    def scale_contour_degenerate(
        ...
        return self.scale_vertices_degenerate(
            contour.vertices, factor_x, factor_y, multipoint_cls, point_cls, segment_cls
        )
            ...
            multipolygon_cls(
                [
                    self.scale_polygon_non_degenerate(
                        polygon, factor_x, factor_y, contour_cls, point_cls, polygon_cls
                    )
    ...
    def scale_multisegment(
        ...
        for segment in multisegment.segments:
            if (
                (factor_x or not is_segment_horizontal(segment))
                and factor_y
                or factor_x
                and not is_segment_vertical(segment)
            ):
        ...
        return pack_mix(
            pack_points(list(unique_ever_seen(scaled_points)), empty, multipoint_cls),
    ...
    def scale_polygon(
        ...
        return (
            ...
            if factor_x and factor_y
                ...
                polygon, factor_x, factor_y, multipoint_cls, point_cls, segment_cls
            )
        )
    ...
    def scale_polygon_degenerate(
        ...
        return self.scale_contour_degenerate(
            polygon.border, factor_x, factor_y, multipoint_cls, point_cls, segment_cls
        )
    ...
    def scale_vertices_projecting_on_ox(
        ...
        for vertex in vertices:
            if min_x > vertex.x:
                ...
                max_x = vertex.x
        return segment_cls(
            point_cls(min_x * factor_x, factor_y), point_cls(max_x * factor_x, factor_y)
        )
    ...
    def scale_vertices_projecting_on_oy(
        ...
        for vertex in vertices:
            if min_y > vertex.y:
                ...
                max_y = vertex.y
        return segment_cls(
            point_cls(factor_x, min_y * factor_y), point_cls(factor_x, max_y * factor_y)
        )
...
unique_ever_seen = dict.fromkeys if sys.version_info >= (3, 6) else OrderedDict.fromkeys
