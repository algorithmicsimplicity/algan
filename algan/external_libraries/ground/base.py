...
class Context:
    ...
    def __init__(
        ...
        *,
        ...
        multipoint_cls: _Type[_hints.Multipoint] = _geometries.Multipoint,
        multipolygon_cls: _Type[_hints.Multipolygon] = _geometries.Multipolygon,
        ...
        (
            ...
            (
                _angular.exact_context,
                _centroidal.exact_context,
                _circular.exact_context,
                _measured.exact_context,
                _metric.exact_context,
                _rotation.exact_context,
                _scaling.exact_context,
                _segment.exact_context,
                _translation.exact_context,
            ...
            if mode is Mode.EXACT
                ...
                if mode is Mode.PLAIN
                    ...
                    _angular.robust_context,
                    _centroidal.robust_context,
                    _circular.robust_context,
                    _measured.robust_context,
                    _metric.robust_context,
                    _rotation.robust_context,
                    _scaling.robust_context,
                    _segment.exact_context,
                    _translation.robust_context,
    ...
    __repr__ = _generate_repr(
        __init__, argument_serializer=_serializers.complex_, skip_defaults=True
    )
    ...
    def angle_kind(self) -> _TernaryPointFunction[Kind]:
        ...
        >>> (context.angle_kind(Point(0, 0), Point(1, 0), Point(-1, 0)) is Kind.OBTUSE)
        True
        >>> (context.angle_kind(Point(0, 0), Point(1, 0), Point(0, 1)) is Kind.RIGHT)
        True
        >>> (context.angle_kind(Point(0, 0), Point(1, 0), Point(1, 0)) is Kind.ACUTE)
        True
        """
    ...
    def box_point_squared_distance(self) -> _metric.BoxPointMetric:
        ...
        >>> context.box_point_squared_distance(Box(0, 1, 0, 1), Point(1, 1)) == 0
        True
        >>> context.box_point_squared_distance(Box(0, 1, 0, 1), Point(2, 1)) == 1
        True
        >>> context.box_point_squared_distance(Box(0, 1, 0, 1), Point(2, 2)) == 2
        True
        """
    ...
    def dot_product(self) -> _QuaternaryFunction:
        ...
        ... ) == -1
        True
        >>> context.dot_product(Point(0, 0), Point(1, 0), Point(0, 0), Point(0, 1)) == 0
        True
        >>> context.dot_product(Point(0, 0), Point(1, 0), Point(0, 0), Point(1, 0)) == 1
        True
        """
    ...
    def locate_point_in_point_point_point_circle(
        self,
    ) -> _circular.PointPointPointLocator:
        """
    ...
    def region_signed_area(self):
        ...
        >>> (
        ...     context.region_signed_area(
        ...         Contour([Point(0, 0), Point(0, 1), Point(1, 1), Point(1, 0)])
        ...     )
        ...     == -1
        ... )
        True
    ...
    def box_segment_squared_distance(
        self, box: _hints.Box, segment: _hints.Segment
    ) -> _hints.Scalar:
        """
        ...
        return self._metric.box_segment_squared_metric(
            box, segment, self.dot_product, self._segments_intersect, self.point_cls
        )
    ...
    def contour_centroid(self, contour: _hints.Contour) -> _hints.Point:
        ...
        True
        """
        return self._centroidal.contour_centroid(contour, self.point_cls, self.sqrt)
    ...
    def contour_length(self, contour: _hints.Contour) -> _hints.Scalar:
        ...
        return sum(
            sqrt(points_squared_distance(vertices[index - 1], vertices[index]))
            for index in range(len(vertices))
        )

    def contour_segments(self, contour: _hints.Contour) -> _Sequence[_hints.Segment]:
        """
 ...
 for contour in contours)``.
    ...
    def merged_box(self, first_box: _hints.Box, second_box: _hints.Box) -> _hints.Box:
        """
        Merges two boxes.
        ...
        >>> context = get_context()
        >>> Box = context.box_cls
        >>> (context.merged_box(Box(0, 1, 0, 1), Box(1, 2, 1, 2)) == Box(0, 2, 0, 2))
        True
        """
        return self.box_cls(
    ...
    def multipoint_centroid(self, multipoint: _hints.Multipoint) -> _hints.Point:
        """
        ...
        ...     Multipoint([Point(0, 0), Point(2, 0), Point(2, 2), Point(0, 2)])
        ... ) == Point(1, 1)
        True
        """
        return self._centroidal.multipoint_centroid(multipoint, self.point_cls)

    def multipolygon_centroid(self, multipolygon: _hints.Multipolygon) -> _hints.Point:
        """
 ...
 for polygon in multipolygon.polygons)``.
        ...
        True
        """
        return self._centroidal.multipolygon_centroid(multipolygon, self.point_cls)

    def multisegment_centroid(self, multisegment: _hints.Multisegment) -> _hints.Point:
        """
    ...
    def multisegment_length(self, multisegment: _hints.Multisegment) -> _hints.Scalar:
        """
    ...
    def points_convex_hull(
        self, points: _Sequence[_hints.Point]
    ...
    def points_box(self, points: _Sequence[_hints.Point]) -> _hints.Box:
        ...
        >>> Box, Point = context.box_cls, context.point_cls
        >>> (
        ...     context.points_box([Point(0, 0), Point(2, 0), Point(2, 2), Point(0, 2)])
        ...     == Box(0, 2, 0, 2)
        ... )
        True
        """
 ...
 for polygon in polygons)``.
    ...
    def replace(
        ...
        *,
        ...
        return Context(
            ...
            contour_cls=(self.contour_cls if contour_cls is None else contour_cls),
            empty_cls=(self.empty_cls if empty_cls is None else empty_cls),
            mix_cls=self.mix_cls if mix_cls is None else mix_cls,
            multipoint_cls=(
                self.multipoint_cls if multipoint_cls is None else multipoint_cls
            ),
            multipolygon_cls=(
                self.multipolygon_cls if multipolygon_cls is None else multipolygon_cls
            ),
            multisegment_cls=(
                self.multisegment_cls if multisegment_cls is None else multisegment_cls
            ),
            point_cls=(self.point_cls if point_cls is None else point_cls),
            polygon_cls=(self.polygon_cls if polygon_cls is None else polygon_cls),
            segment_cls=(self.segment_cls if segment_cls is None else segment_cls),
    ...
    def rotate_contour(
        ...
        ...         Contour([Point(0, 0), Point(1, 0), Point(0, 1)]), 1, 0, Point(0, 1)
        ...     )
        ...
        >>> (
        ...     context.rotate_contour(
        ...         Contour([Point(0, 0), Point(1, 0), Point(0, 1)]), 0, 1, Point(0, 1)
        ...     )
    ...
    def rotate_contour_around_origin(
        self, contour: _hints.Contour, cosine: _hints.Scalar, sine: _hints.Scalar
    ) -> _hints.Contour:
        """
    ...
    def rotate_multipoint_around_origin(
        self, multipoint: _hints.Multipoint, cosine: _hints.Scalar, sine: _hints.Scalar
    ) -> _hints.Multipoint:
        """
 ...
 for polygon in multipolygon.polygons)``.
    ...
    def rotate_point_around_origin(
        self, point: _hints.Point, cosine: _hints.Scalar, sine: _hints.Scalar
    ) -> _hints.Point:
        """
        ...
        >>> context = get_context()
        >>> Point = context.point_cls
        >>> (context.rotate_point_around_origin(Point(1, 0), 1, 0) == Point(1, 0))
        True
        >>> (context.rotate_point_around_origin(Point(1, 0), 0, 1) == Point(0, 1))
        True
        """
        return self._rotation.rotate_point_around_origin(
 ...
 + sum(len(hole.vertices) for hole in polygon.holes)``.
    ...
    def rotate_polygon_around_origin(
        self, polygon: _hints.Polygon, cosine: _hints.Scalar, sine: _hints.Scalar
    ) -> _hints.Polygon:
        """
 ...
 + sum(len(hole.vertices) for hole in polygon.holes)``.
        ...
        return self._rotation.rotate_polygon_around_origin(
            polygon, cosine, sine, self.contour_cls, self.point_cls, self.polygon_cls
        )
    ...
    def rotate_segment_around_origin(
        self, segment: _hints.Segment, cosine: _hints.Scalar, sine: _hints.Scalar
    ) -> _hints.Segment:
        """
    ...
    def scale_contour(
        self, contour: _hints.Contour, factor_x: _hints.Scalar, factor_y: _hints.Scalar
    ...
    def scale_multipoint(
        ...
        >>> Point = context.point_cls
        >>> (
        ...     context.scale_multipoint(Multipoint([Point(0, 0), Point(1, 1)]), 0, 0)
        ...     == Multipoint([Point(0, 0)])
        ... )
        True
        >>> (
        ...     context.scale_multipoint(Multipoint([Point(0, 0), Point(1, 1)]), 1, 0)
        ...     == Multipoint([Point(0, 0), Point(1, 0)])
        ... )
        True
        >>> (
        ...     context.scale_multipoint(Multipoint([Point(0, 0), Point(1, 1)]), 0, 1)
        ...     == Multipoint([Point(0, 0), Point(0, 1)])
        ... )
        True
        >>> (
        ...     context.scale_multipoint(Multipoint([Point(0, 0), Point(1, 1)]), 1, 1)
        ...     == Multipoint([Point(0, 0), Point(1, 1)])
        ... )
        True
        """
        return self._scaling.scale_multipoint(
 ...
 for polygon in multipolygon.polygons)``.
    ...
    def scale_multisegment(
        ...
        ...     )
        ...     == Mix(
        ...         Multipoint([Point(0, 0)]), Segment(Point(0, 0), Point(1, 0)), EMPTY
        ...     )
        ... )
        True
        >>> (
        ...
        ...     )
        ...     == Mix(
        ...         Multipoint([Point(0, 0)]), Segment(Point(0, 0), Point(0, 1)), EMPTY
        ...     )
        ... )
        True
        >>> (
    ...
    def scale_point(
        self, point: _hints.Point, factor_x: _hints.Scalar, factor_y: _hints.Scalar
    ) -> _hints.Point:
        """
        ...
        True
        """
        return self._scaling.scale_point(point, factor_x, factor_y, self.point_cls)

    def scale_polygon(
        self, polygon: _hints.Polygon, factor_x: _hints.Scalar, factor_y: _hints.Scalar
 ...
 + sum(len(hole.vertices) for hole in polygon.holes)``.
    ...
    def scale_segment(
        self, segment: _hints.Segment, factor_x: _hints.Scalar, factor_y: _hints.Scalar
    ...
    def segment_box(self, segment: _hints.Segment) -> _hints.Box:
        ...
        ...     context.segment_cls,
        ... )
        >>> (context.segment_box(Segment(Point(0, 1), Point(2, 3))) == Box(0, 2, 1, 3))
        True
        """
    ...
    def segment_centroid(self, segment: _hints.Segment) -> _hints.Point:
        ...
        >>> (context.segment_centroid(Segment(Point(0, 1), Point(2, 3))) == Point(1, 2))
        True
        """
        return self._centroidal.segment_centroid(segment, self.point_cls)

    def segment_contains_point(
        self, segment: _hints.Segment, point: _hints.Point
    ) -> bool:
        """
        ...
        False
        """
        return self._segment.containment_checker(
    ...
    def segment_length(self, segment: _hints.Segment) -> _hints.Scalar:
        ...
        True
        """
        return self.sqrt(self.points_squared_distance(segment.start, segment.end))
    ...
    def segments_intersection(
        self, first: _hints.Segment, second: _hints.Segment
    ) -> _hints.Point:
        """
        ...
        ...         Segment(Point(0, 0), Point(2, 0)), Segment(Point(0, 0), Point(0, 1))
        ...     )
        ...     == Point(0, 0)
        ... )
        True
        >>> (
        ...     context.segments_intersection(
        ...         Segment(Point(0, 0), Point(2, 0)), Segment(Point(1, 0), Point(1, 1))
        ...     )
        ...     == Point(1, 0)
        ... )
        True
        >>> (
        ...     context.segments_intersection(
        ...         Segment(Point(0, 0), Point(2, 0)), Segment(Point(2, 0), Point(3, 0))
        ...     )
        ...     == Point(2, 0)
        ... )
        True
        """
        return self._segment.intersector(
            first.start,
            first.end,
            second.start,
            second.end,
    ...
    def segments_relation(self, test: _hints.Segment, goal: _hints.Segment) -> Relation:
        """
        ...
        ...         Segment(Point(0, 0), Point(2, 2)), Segment(Point(1, 0), Point(2, 0))
        ...     )
        ...
        >>> (
        ...     context.segments_relation(
        ...         Segment(Point(0, 0), Point(2, 2)), Segment(Point(0, 0), Point(2, 0))
        ...     )
        ...     is Relation.TOUCH
        ... )
        True
        >>> (
        ...     context.segments_relation(
        ...         Segment(Point(0, 0), Point(2, 2)), Segment(Point(2, 0), Point(0, 2))
        ...     )
        ...     is Relation.CROSS
        ... )
        True
        >>> (
        ...     context.segments_relation(
        ...         Segment(Point(0, 0), Point(2, 2)), Segment(Point(0, 0), Point(1, 1))
        ...     )
        ...
        >>> (
        ...     context.segments_relation(
        ...         Segment(Point(0, 0), Point(2, 2)), Segment(Point(0, 0), Point(2, 2))
        ...     )
        ...     is Relation.EQUAL
        ... )
        True
        >>> (
        ...     context.segments_relation(
        ...         Segment(Point(0, 0), Point(2, 2)), Segment(Point(0, 0), Point(3, 3))
        ...     )
        ...
        >>> (
        ...     context.segments_relation(
        ...         Segment(Point(0, 0), Point(2, 2)), Segment(Point(1, 1), Point(3, 3))
        ...     )
        ...     is Relation.OVERLAP
        ... )
        True
        """
        return self._segment.relater(
    ...
    def segments_squared_distance(
        ...
        ...     Segment(Point(0, 0), Point(1, 0)), Segment(Point(0, 0), Point(0, 1))
        ... ) == 0
        True
        >>> context.segments_squared_distance(
        ...
        True
        >>> context.segments_squared_distance(
        ...     Segment(Point(0, 0), Point(1, 0)), Segment(Point(2, 1), Point(2, 2))
        ... ) == 2
        True
        """
    ...
    def translate_contour(
        self, contour: _hints.Contour, step_x: _hints.Scalar, step_y: _hints.Scalar
    ) -> _hints.Contour:
        """
 ...
 for polygon in multipolygon.polygons)``.
    ...
    def translate_point(
        self, point: _hints.Point, step_x: _hints.Scalar, step_y: _hints.Scalar
    ) -> _hints.Point:
        """
        ...
        True
        """
        return self._translation.translate_point(point, step_x, step_y, self.point_cls)

    def translate_polygon(
        self, polygon: _hints.Polygon, step_x: _hints.Scalar, step_y: _hints.Scalar
    ) -> _hints.Polygon:
        """
 ...
 + sum(len(hole.vertices) for hole in polygon.holes)``.
        ...
        return self._translation.translate_polygon(
            polygon, step_x, step_y, self.contour_cls, self.point_cls, self.polygon_cls
        )

    def translate_segment(
        self, segment: _hints.Segment, step_x: _hints.Scalar, step_y: _hints.Scalar
    ) -> _hints.Segment:
        """
    ...
    def _segment_contains_point(
        self, start: _hints.Point, end: _hints.Point, point: _hints.Point
    ) -> bool:
        return self._segment.containment_checker(
    ...
    def _segments_intersect(
        ...
        return self._segment.collision_detector(
            first_start, first_end, second_start, second_end, self.angle_orientation
        )


_context = _ContextVar("context", default=Context())
...
def set_context(context: Context) -> None:
    ...
    assert isinstance(context, Context), (
        'Expected "{expected}" instance, but got "{actual}".'.format(
