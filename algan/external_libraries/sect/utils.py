...
from typing import FrozenSet, Iterable, Sequence, Tuple, Type

from algan.external_libraries.ground.base import Context, Orientation
from algan.external_libraries.ground.hints import Contour, Point, Segment

from algan.external_libraries.sect.core.utils import (
    arg_min,
    flatten,
    rotate_sequence,
    to_contour_orientation,
)
from .hints import SegmentContainmentChecker, SegmentEndpoints
...
def complete_vertices(
    ...
    border, candidates = _complete_contour_vertices(
        border, candidates, contour_cls, segment_cls, segment_point_relater
    )
    completed_holes = []
    for hole in holes:
        hole, candidates = _complete_contour_vertices(
            hole, candidates, contour_cls, segment_cls, segment_point_relater
        )
...
def contour_to_oriented_edges_endpoints(
    contour: Contour, *, clockwise: bool, orienteer
) -> Iterable[SegmentEndpoints]:
    vertices = contour.vertices
    return (
        ((vertices[index - 1], vertices[index]) for index in range(len(vertices)))
        if (
            to_contour_orientation(contour, orienteer)
            is (Orientation.CLOCKWISE if clockwise else Orientation.COUNTERCLOCKWISE)
        )
        else (
...
def normalize_contour_vertices(vertices: Sequence[Point], orienteer) -> Sequence[Point]:
    vertices = rotate_sequence(vertices, arg_min(vertices))
    return (
        vertices[:1] + vertices[1:][::-1]
        if (orienteer(vertices[-1], vertices[0], vertices[1]) is Orientation.CLOCKWISE)
        else vertices
    )
...
def _complete_contour_vertices(
    ...
    for index, end in enumerate(vertices):
        start_index, end_index = (bisect(candidates, start), bisect(candidates, end))
        if start_index > end_index:
            start_index, end_index = end_index, start_index
        extra_vertices_indices = [
            candidate_index
            for candidate_index in range(start_index, end_index)
            if _is_inner_segment_point(
                start,
                end,
        ...
        if extra_vertices_indices:
            extra_vertices[index] = [
                candidates[index] for index in extra_vertices_indices
            ]
