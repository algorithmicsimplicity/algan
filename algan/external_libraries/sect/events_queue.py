...
from algan.external_libraries.ground.base import Context, Orientation, Relation
...
class EventsQueueKey:
    ...
    def __init__(self, orienteer, event: Event) -> None:
    ...
    def __lt__(self, other: EventsQueueKey) -> bool:
        ...
        # or both are right endpoints
            ...
            other_end_orientation = self.orienteer(
                event.start, event.end, other_event.end
            )
...
class EventsQueue:
    ...
    def __init__(self, context: Context) -> None:
        ...
        self._queue = PriorityQueue(
            key=partial(EventsQueueKey, context.angle_orientation)
        )

    @staticmethod
    def compute_position(below_event: Optional[LeftEvent], event: LeftEvent) -> None:
        if below_event is not None:
            event.other_interior_to_left = (
                below_event.other_interior_to_left
                if (event.from_first is below_event.from_first)
                else below_event.interior_to_left
            )

    def detect_intersection(self, below_event: LeftEvent, event: LeftEvent) -> bool:
        """
        ...
        if relation is Relation.CROSS or relation is Relation.TOUCH:
            ...
            start_min, start_max = (
                (event, below_event)
                if starts_equal
                or (self._queue.key(event) < self._queue.key(below_event))
                else (below_event, event)
            )
            end_min, end_max = (
                (event.right, below_event.right)
                if ends_equal
                or (self._queue.key(event.right) < self._queue.key(below_event.right))
    ...
    def register_edge(
        self, edge: QuadEdge, *, from_first: bool, is_counterclockwise_contour: bool
    ) -> None:
        ...
        while queue:
            ...
            if event.is_left:
                ...
                above_event, below_event = (
                    sweep_line.above(event),
                    sweep_line.below(event),
                )
                self.compute_position(below_event, event)
                if above_event is not None and self.detect_intersection(
                    event, above_event
                ):
                    self.compute_position(event, above_event)
                if below_event is not None and self.detect_intersection(
                    below_event, event
                ):
                    self.compute_position(sweep_line.below(below_event), below_event)
            else:
                event = event.left
