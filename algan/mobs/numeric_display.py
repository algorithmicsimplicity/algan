...
class NumericDisplay(Mob):
    def __init__(self, value, num_decimal_places=2, **kwargs):
        ...
        with Off():
            with NoExtra(priority_level=1):
                self.placeholder = Tex(f'-0.{"".join(["0" for _ in range(num_decimal_places)])}', **kwargs)
                ...
                for i in range(len(self.digit_mobs)):
                    self.digit_mobs[i].character_mobs.location = self.placeholder[i+2 if i > 0 else 1].location
                self._value = value
    ...
    def update_display(self, value):
        ...
        def get_opacities(value):
            ...
            for v in value:
                ...
                for i, digit in enumerate(value_digits):
                    digit = int(digit)
                    all_opacities.append(F.one_hot(torch.tensor((digit,)), 10).transpose(0,1))
        ...
        with Sync():
            ...
            for i in range(len(self.digit_mobs)):
                prep(self.digit_mobs[i].character_mobs).set(opacity=all_opacities[i], max_opacity=all_opacities[i])
