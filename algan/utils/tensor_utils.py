...
def packed_reorder(x, counts, ids):
    ...
    s = ids_rep.argsort(0)
    cc_rep + (torch.arange(len(s)) - cc_rep[s])[ids_rep]
    return ids
...
def make_grid(
    height, width=None, min_coord=-1, max_coord=1, min_coord2=None, max_coord2=None
):
    if width is None:
        width = height
    ...
    return torch.stack(
        (
            ...
            torch.linspace(min_coord, max_coord, height, device=device)
            .view(-1, 1)
            .expand(torch.Size([-1, width])),
            torch.linspace(min_coord2, max_coord2, width, device=device)
            .view(1, -1)
            .expand(torch.Size([height, -1])),
        ),
        -1,
    )
...
def unsqueeze_right(x, y, offset: int = 0):
    if not isinstance(x, torch.Tensor):
        return x
    return x.view(
        [1] * offset + list(x.shape) + [1] * (len(y.shape) - (len(x.shape) + offset))
    )
...
def unsqueeze_left(x, y, offset: int = 0):
    if not isinstance(x, torch.Tensor):
        return x
    return x.view(
        *([1] * (len(y.shape) - (len(x.shape) + offset))),
        *([1] * offset + list(x.shape)),
    )
...
def broadcast_all(xs, ignored_dims=[]):
    ...
    xs = [
        unsqueeze_until_dim(x, max_dim) if isinstance(x, torch.Tensor) else x
        for x in xs
    ]
    max_shapes = [
        max([x.shape[i] if isinstance(x, torch.Tensor) else 0 for x in xs])
        if i not in ignored_dims
        else -1
        for i in range(max_dim)
    ]
    return [
        x.expand(torch.Size(max_shapes)) if isinstance(x, torch.Tensor) else x
        for x in xs
    ]
...
def broadcast_scatter(_input, dim, ind, src, **kwargs):
    _input, ind, src = broadcast_all(
        [_input, ind, src], ignored_dims=[dim if dim >= 0 else len(src.shape) + dim]
    )
    return _input.scatter_reduce(dim, ind, src, **kwargs)
...
def broadcast_cross_product(x, y, dim=-1):
    x, y = broadcast_both_left(
        x, y, ignored_dims=[dim]
    )  # [dim if dim >= 0 else len(y.shape) + dim])
...
def broadcast_interleave(x, counts, inds, dim=-2):
    x = broadcast_gather(
        x, dim, unsqueeze_right(inds.clamp_max(x.shape[dim] - 1), x, dim), keepdim=True
    )
...
def robust_concat(xs):
    ...
    max_shape = torch.stack(
        [unsqueeze_left_to_max_dim(torch.tensor(x.shape)) for x in xs]
    ).amax(0)
    xs = [x.view(*([1] * (max_dim - x.dim())), *x.shape) for x in xs]
    return torch.cat(
        [
            x.expand([_ if x.shape[i] == 1 else -1 for i, _ in enumerate(max_shape)])
            for x in xs
        ]
    )
...
def concat_dicts(kwargs):
    ...
    return {
        a: (
            robust_concat([kwargs[i][a] for i in range(len(kwargs))])
            if (isinstance(kwargs[0][a], torch.Tensor))
            else kwargs[0][a]
        )
        for a in kwargs[0]
    }
