...
class ModificationHistory:
    ...
    def get_history(self, animatable):
        ...
        for _func_name_outer, func_data_outer in sorted(
            self.function_applications.items(),
            key=lambda _: (
                0 if "setattr_" in _[0] else (1 if "update_relative" in _[0] else 2)
            ),
        ):
            for _func_name, func_data in self.function_applications.items():
        ...
        for _func_name, (func, _modified_attrs, arg_list, _execution_order) in sorted(
            self.function_applications.items(),
            key=lambda _: (
                0 if "setattr_" in _[0] else (1 if "update_relative" in _[0] else 2)
            ),
            ...
            for order in unique_orders:  # [-1:]:
                ...
                def g(x, uinds=uinds, order=order):
                ...
                funcs.append(
                    (
                        func,
                        {
                            k: g(v, uinds=uinds, order=order)
                            for k, v in animated_args.items()
                        },
                        {k: g(v, uinds=uinds, order=order) for k, v in kwargs.items()},
                        g(start_time, uinds=uinds, order=order),
                        g(end_time, uinds=uinds, order=order),
                        g(extended_end_time, uinds=uinds, order=order),
                        sub_rate_funcs,
                    )
                )
        return attrs, funcs
...
def animated_function(
    function=None, *, animated_args: dict[str, float] = {}, unique_args=[]
):
...
class Animatable:
    ...
    def _set_dependant_mobs_time_inds_to_self_then_run_function(self, function):
        ...
        for mob in dependent_mobs:
            ...
            if hasattr(self, "raw_s") and (mob.set_state_full(self.raw_s, self.raw_e)):
    ...
    def set_state_full(self, s, e):
        ...
        for (
            ...
            animated_args = select_kwargs(animated_args, fa)
