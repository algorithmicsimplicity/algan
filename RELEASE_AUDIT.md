...
> **Resolved by option (a).** `algan/external_libraries/manim` is now Manim
> Community 0.21.0 cut down to the geometry Algan converts, with every
> `from manim...` import rewritten relative — regenerated by
> `scripts/vendor_manim.py`, documented in that directory's `VENDORING.md`, and
> the script fails the build if one absolute import survives.
> `algan/external_libraries/manim_alias.py` registers it as `manim` through a
> `sys.meta_path` finder (aliasing the package alone would let a submodule
> execute twice under two names and define two of every class).
> `manim`, `cloup` and `pycairo` are gone from `dependencies`, which drops 16
> packages from the lock; `networkx` and `typing-extensions` are added because
