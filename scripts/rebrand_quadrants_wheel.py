...
``1.3.0.post1`` so CMake and Python agree on the downstream version.  This
...
DOWNSTREAM_VERSION = "1.3.0.post1"
...
def _renamed_info(info: zipfile.ZipInfo, name: str) -> zipfile.ZipInfo:
    """Clone a ZipInfo while changing only its member name."""
...
def rebrand_wheel(path: Path, *, delete_original: bool = False) -> Path:
    """Return a wheel whose distribution is ``algan-quadrants``.

    The wheel must already have the downstream version.  That requirement is
    deliberate: Quadrants feeds the project version into CMake, so changing the
    version only after compilation could make the installed metadata disagree
...
def validate_downstream_wheel(path: Path) -> None:
    """Validate the downstream name/version and retained ``quadrants`` import."""
    path = Path(path)
    distribution, version, _ = _wheel_parts(path)
    if distribution != DOWNSTREAM_WHEEL_DISTRIBUTION:
        raise ValueError(
            f"{path.name}: expected wheel distribution {DOWNSTREAM_WHEEL_DISTRIBUTION!r}"
        )
    if version != DOWNSTREAM_VERSION:
        raise ValueError(
            f"{path.name}: expected version {DOWNSTREAM_VERSION!r}, got {version!r}"
...
def main() -> int:
    ...
    parser.add_argument(
        "--delete-original",
        action="store_true",
        help="remove each upstream-named wheel after a successful rewrite",
    )
    parser.add_argument(
        "--validate-only",
        action="store_true",
        help="validate already-rebranded wheels instead of rewriting them",
    )
