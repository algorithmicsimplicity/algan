...
- `samples_per_pixel > 1` selects the Monte Carlo path tracer (`path_tracer.py` drives `path_tracer_taichi.py`'s `pt_generate` / `pt_shade` / `pt_reduce` in the same wavefront shape, sharing `wavefront_traverse_events`), which has its own cold compile. It provides indirect transport and bounded, non-splitting per-path state. This is an **explicit selection**, not an automatic retry after a deterministic memory failure. Geometry, acceleration structures, event buffers and output storage still consume memory. `../algan/rendering/raytracing/DESIGN_path_tracer_roadmap.md` describes its implementation and remaining scope. Homogeneous scattering is rejected on the deterministic route by default; `_build_render_plan` and the unsupported-feature policy are the source of truth for capability checks.

The deterministic renderer resolves primary visibility through the sheet route (`../algan/rendering/raytracing/DESIGN_sheet_resolve.md`) when the batch qualifies: analytic-coverage fragments are emitted for flat triangles and flattened Bezier circuits, compacted by PyTorch/Taichi passes into per-pixel depth-banded sheets (`sheets.py`), and resolved/shaded — shadow events included — by the one kernel body in `sheet_resolve_taichi.py`, while reflection/refraction continuations remain in the ray-based wavefront system. It is the only analytic-coverage resolve; batches the route rejects (including analytic AA off, transparent background with an env map, positive near clipping, custom scatter, or route toggles off) render through the classic supersampled wavefront when SPP is 1. Higher SPP selects the path tracer described above. `analytic_raster_route_active` in `tracer.py` is the single host-side route decision shared by allocation planning and rendering. Do not describe the current renderer as either a pure rasterizer or a pure one-primary-ray-per-pixel tracer.
...
The settings facade resolves fields across the owning rendering modules, including `raytracing/settings.py`, with env-var defaults and setters where needed. Public output controls live on `SETTINGS.raytracing`; low-level tuning lives on its `.experimental` section. **Read them live** (`rt_settings.samples_per_pixel`, for example, at call time) — importing them by value at module import freezes them before user code runs (this bug has shipped before).

Post-processing (`post_processing/`): bloom/glow, optional FXAA and tonemapping; the standalone SMAA code is not a normal post-processing route. `Camera` (`camera.py`): perspective with a near-orthographic approximation, fov/near/far; render code consumes an immutable camera/light snapshot per batch so batch prep for frame batch N+1 can run on a worker thread while N renders (`ALGAN_PREFETCH_BATCHES=0` disables).

## Arena-packed kernel arguments

The original Metal port exceeded the backend's buffer-argument budget in wide shade/traverse kernels. Those kernels now pass cold arrays as offsets into the `ManualMemory` arena; `algan/rendering/raytracing/arena_args_taichi.py` defines the packed views. Argument counts evolve with the signatures and compiler; use the binding tests and actual signatures rather than the migration's old maxima.
...
- The cost is Taichi re-loading base pointers and shapes from a global-memory argument buffer at every use site. The PTX and the measurements are in `DESIGN_taichi_argument_loads.md`, which is deleted — `git show aa7d198^:DESIGN_taichi_argument_loads.md`. The corresponding compiler work is `quadrants_patches/0004-llvm-invariant-load-kernel-args.patch` (`!invariant.load` on argument loads). See `quadrants_patches/README.md` for patch provenance and measured results; remeasure on the target backend rather than treating historical CPU evidence as a current CUDA result.
...
Physical transmitting triangles mix single-scatter GGX visible-facet/Fresnel
sampling with a reciprocal two-sided energy-closure lobe. `_pt_glass_terms`
supplies the single-facet outcomes; `_pt_sample_glass` and `_pt_glass_f_pdf`
must agree on the complete mixture probabilities and direction density,
...
The coupled glass correction restores power but approximates higher-order
angular transport. Keep the VNDF/cosine mixture PDF and VNDF null events
consistent at both ends of MIS. Its loss table follows the optional blue-noise
tile in `nee_meta`; header word 22 records the offset. See the roadmap's
"Coupled rough-glass compensation" record before changing its normalization. Shadow connections through further
