...
- **Edit the canonical layout, then regenerate.** `raytracing/arena_layouts.py` lists each packed kernel's original argument order once, with `(name, dtype tag, rank)` for bound arrays. Run `python scripts/generate_arena_bindings.py` to regenerate the literal prologues, `_<KERNEL>_ARENA` specs and `_<KERNEL>_PARAMS` lists. Do not hand-renumber `aoff`/`ashp`. Ordinary kept arguments still need their kernel signature and explicit launch sites updated; the generator validates the signature before writing, and `tests/unit_tests/test_arena_args.py` independently checks layouts and call-site arity. `--check` and `test_arena_layout_codegen.py` reject stale generated regions.
