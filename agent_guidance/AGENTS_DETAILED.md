...
- The client half declines the handoff outright when something is debugging or tracing the process — pydevd/debugpy/ptvsd in `sys.modules`, a `sys.monitoring` debugger, or any `sys.gettrace()` function at all, `coverage` included (`debugger_name` in `../algan/daemon_client.py`). The daemon would run the script in a process the debugger is not attached to, so every breakpoint would be skipped silently. It says which way it went, and an explicit `ALGAN_USE_DAEMON=1` forces the handoff anyway — for the one warm-and-debuggable arrangement, a daemon that is itself launched under the debugger. Client runs execute on the daemon's *main* thread, which is why that works.
