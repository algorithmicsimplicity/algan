...
- **Pixel comparisons do run.** Baselines are per device and **both** `expected_outputs_cpu/` and `expected_outputs_cuda/` are committed (7 files each), so `tests/fast` and `tests/full_renders` compare against the CPU set here rather than skipping. A renderer change *is* validated in a cloud session, on the CPU path. What you still cannot check here is CUDA-specific behaviour and CPU/CUDA divergence (`tests/README.md` documents PN-surface and silhouette differences), so a kernel change still wants a CUDA machine before it ships. A suite that finds no baseline directory for its device renders the scene and silently skips (`no <device> fast-suite baseline is available`) — read the skip reasons rather than assuming a pass.
- **Re-baseline here only deliberately.** `ALGAN_UPDATE_*_BASELINE=1` writes `expected_outputs_cpu/` only — it cannot touch the CUDA set, which has to be regenerated on a CUDA machine, so a change that moves output needs *both* before it is complete. Never re-baseline to turn a red test green: confirm the visual change is the one you intended (diff videos land in the suite's `output_errors/`) and say in the commit why output moved.
