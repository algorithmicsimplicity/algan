...
def _count(extent, b):
    kernels.count_selected(b.n, b.inputs["accepted"], b.rank, b.totals, extent.header.tensor)
...
def _pack(extent, b):
    ...
    kernels.pack_primary_shadow_events(
        b.n, extent.capacity, extent.header.tensor, b.rank, b.scan[0][1],
        b.scan[-1][2], src["accepted"], src["source"], src["pos"], src["snrm"],
        src["fnrm"], src["frame"], src["mask"], src["dp"], src["toff"],
        out["pos"], out["snrm"], out["fnrm"], out["frame"], out["mask"],
        out["source"], out["dp"], out["toff"], b.reverse,
        int(b.footprint), int(b.terminator))


_PRIMARY_SHADOW_PLAN = DispatchPlan((
    DispatchStage("reset", _reset), DispatchStage("count", _count),
    DispatchStage("scan", _scan), DispatchStage("pack", _pack),
))
...
class PrimaryShadowDispatch:
    ...
    def run(self, trace, shadow_vis):
        output = TypedArenaView.bind(shadow_vis, name="shadow_vis", access="write",
                                     dtype=torch.float32, ndim=3)
        ...
        return _PRIMARY_SHADOW_PLAN.run(
            self.extent, self.views + (output,), self,
            consume=lambda _extent, _bindings, _metadata: trace(),
            synchronize=lambda: _synchronize(shadow_vis.device))


def prepare_primary_shadow_dispatch(memory, *, n, accepted, source, pos, snrm,
                                    fnrm, frame, mask, dp, toff, reverse,
                                    footprint, terminator, capacity=None):
    ...
    if isinstance(capacity, bool) or not isinstance(capacity, int) or not 0 <= capacity <= (1 << 31) - 1:
        raise ValueError("Queue capacity must fit signed int32")
    allocate = memory.get_tensor
    inputs = dict(accepted=accepted, source=source, pos=pos, snrm=snrm, fnrm=fnrm,
                  frame=frame, mask=mask, dp=dp, toff=toff)
    views = []
    for name, tensor in inputs.items():
        width = 6 if name == "dp" else 3 if name in ("pos", "snrm", "fnrm", "toff") else 1
        dtype = torch.float32 if width > 1 else torch.int32
        view = TypedArenaView.bind(tensor, name=name, access="read", dtype=dtype,
                                   ndim=2 if width > 1 else 1, vector_width=width)
        required = (n if (name != "dp" or footprint) and
                    (name != "toff" or terminator) else 1)
    ...
    for name in ("pos", "snrm", "fnrm", "frame", "mask", "source", "dp", "toff"):
        ...
        if (name == "dp" and not footprint) or (name == "toff" and not terminator):
            size = 1
        tensor = allocate((size, width) if width > 1 else (size,),
    ...
    header = TypedArenaView.bind(allocate((4,), torch.int32), name="header", access="readwrite")
    ...
    views.extend((TypedArenaView.bind(rank, name="rank", access="readwrite"),
    ...
    while True:
        ...
        views.extend((TypedArenaView.bind(offsets, name=f"offsets_{level}", access="readwrite"),
                      TypedArenaView.bind(sums, name=f"sums_{level}", access="readwrite")))
        if parents == 1:
            break
        nb, level = parents, level + 1
    return PrimaryShadowDispatch(n, inputs, outputs, reverse, bool(footprint),
                                 bool(terminator), rank, totals, tuple(scan), extent, tuple(views))
