...
class RegionLayout:
    ...
    def from_tensor(cls, tensor):
        ...
        if tensor.layout != torch.strided or not tensor.is_contiguous():
            raise ArenaRegionError("Typed arena regions require contiguous strided tensors")
        ...
        if any(d < 0 or d > _INT32_MAX for d in shape):
        ...
        return cls(_storage_key(tensor), tensor.dtype, shape, strides,
                   offset, length, size, alignment)
    ...
    def overlaps(self, other):
        return (self.storage == other.storage and self.byte_length > 0
                and other.byte_length > 0 and self.byte_offset < other.byte_end
...
class _Allocation:
    ...
    def validate(self):
        ...
        if not self.alive or tracker is None or tracker.owner() is None:
            raise ArenaRegionError("Arena region was reclaimed; bind a fresh allocation")
...
class ArenaRegionTracker:
    ...
    def rewind(self, *, forward=None, reverse=None):
        invalid = [a for a in self.allocations if a.alive and (
            (not a.persistent and forward is not None and a.layout.byte_end > forward)
            or (a.persistent and reverse is not None and a.layout.byte_offset < reverse))]
        if any(a.pins for a in invalid):
            raise ArenaRegionError("Cannot rewind an arena region used by an unfinished dispatch")
...
def _contains(outer, inner):
    return (outer.storage == inner.storage and outer.byte_offset <= inner.byte_offset
...
class TypedArenaView:
    ...
    @classmethod
    def bind(cls, tensor, *, name, access="read", dtype=None, ndim=None, vector_width=1):
        ...
        if vector_width < 1 or (vector_width > 1 and (
                not layout.shape or layout.shape[-1] != vector_width)):
        ...
        allocation = tracker.allocation_for(tensor, layout) if tracker is not None else None
        return cls(tensor, layout, access, name, allocation)

    def validate(self):
        if RegionLayout.from_tensor(self.tensor) != self.layout:
            raise ArenaRegionError(f"{self.name}: tensor metadata changed after binding")
...
def validate_regions(views):
    ...
    for i, a in enumerate(views):
        for b in views[i + 1:]:
            if a.layout.overlaps(b.layout) and (a.access != "read" or b.access != "read"):
                raise ArenaRegionError(f"Writable alias between {a.name} and {b.name}")
    return tuple(RegionLoadMetadata(
        view.name, view.layout, view.access == "read",
        tuple(other.name for other in views if other is not view
              and not view.layout.overlaps(other.layout))) for view in views)
...
def lease_regions(views):
    ...
    allocations = {id(v.allocation): v.allocation for v in views if v.allocation is not None}
